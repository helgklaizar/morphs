...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    pb.collection('tables').getFullList().then(data => setTables(data.filter((t: any) => t.is_active))).catch(console.error);
  }, []);
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const orderRecord = await pb.collection('orders').create({
        customer_name: extendedCustomerName,
        customer_phone: customerPhone || "",
        status: "new",
        total_amount: getTotal(),
        payment_method: paymentMethod,
        table_id: orderType === 'dine_in' && selectedTableId ? selectedTableId : undefined,
      });
      ...
      useToastStore.getState().error("Ошибка при создании заказа: " + (err?.message || String(err)));
    } finally {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] gap-4 lg:gap-6 overflow-hidden">
      {/* Left Column: Menu Items */}
      <div className="flex-1 flex flex-col gap-4 lg:gap-6 overflow-hidden bg-black/20 backdrop-blur-3xl border border-white/5 shadow-2xl rounded-[32px] p-6 lg:p-8 relative">
        ...
        <div className="flex items-center justify-between z-10 shrink-0">
          ...
          {/* Categories */}
          <div className="flex gap-2 overflow-x-auto custom-scrollbar z-10 w-full ml-6 pl-6 border-l border-white/10">
            <button
              onClick={() => {
                setActiveCategory('all');
                setSearchQuery("");
              }}
              className={`px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap border ${
                activeCategory === 'all' 
                  ? 'bg-gradient-to-r from-orange-500 to-orange-400 text-white border-orange-500/50 shadow-[0_0_20px_rgba(249,115,22,0.3)]' 
      ...
      {/* Right Column: Cart */}
      <div className="w-[380px] lg:w-[420px] bg-black/20 backdrop-blur-3xl border border-white/5 rounded-[32px] flex flex-col overflow-hidden shadow-2xl shrink-0 relative">
        <div className="absolute top-[-20%] right-[-20%] w-64 h-64 bg-orange-500/5 rounded-full blur-[100px] pointer-events-none" />
        
        <div className="p-6 lg:p-8 pb-4 border-b border-white/5 bg-transparent flex items-center justify-between shrink-0 z-10">
          <h2 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3">
            <ShoppingBag className="w-6 h-6 text-orange-500" /> Чек
          </h2>
          {cartItems.length > 0 && (
            <button 
              onClick={clearCart}
              className="px-3 py-1.5 rounded-full bg-red-500/10 text-[10px] uppercase tracking-widest text-red-500 hover:bg-red-500/20 font-black transition-colors"
            >
              Очистить
            </button>
          )}
        ...
        {/* Client Config Area */}
        <div className="flex-none overflow-y-auto p-4 lg:p-6 space-y-4 custom-scrollbar bg-white/5 border-t border-white/5 z-10">
        ...
        <div className="p-6 lg:p-8 bg-black/40 backdrop-blur-xl border-t border-white/10 space-y-4 z-20 shrink-0">
          <div className="flex gap-3">
            <button 
              onClick={() => setPaymentMethod('cash')}
              className={`flex-1 flex flex-col items-center justify-center gap-2 py-3 rounded-2xl border transition-all ${
                paymentMethod === 'cash' 
                  ? 'bg-orange-500/10 border-orange-500 text-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.15)]' 
                  : 'bg-white/5 border-white/5 text-white/40 hover:text-white/80 hover:bg-white/10 hover:border-white/10'
              }`}
            >
              <Banknote className="w-6 h-6" />
              <span className="text-[10px] font-black uppercase tracking-widest">Наличные</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">Bit / Карта</span>
            </button>
          </div>

          <div className="flex items-end justify-between py-2 border-y border-white/10">
            <span className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">Итого к оплате</span>
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 disabled:from-white/5 disabled:to-white/5 disabled:text-white/30 text-white font-black py-4 rounded-2xl shadow-[0_0_30px_rgba(249,115,22,0.3)] disabled:shadow-none active:scale-[0.98] transition-all flex items-center justify-center gap-3 group text-lg tracking-wider"
          >
            {isSubmitting ? (
