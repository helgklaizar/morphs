...
class DeployMorph:
    ...
    def generate_docker(self, project_name: str) -> bool:
        """
        Bridges the legacy call in main.py to the deploy artifact generation.
        """
        try:
            workspace_dir = "./workspaces/deploy_morphs_os"
            self.generate_deploy_artifacts(workspace_dir, project_name)
            return True
        except Exception as e:
            logger.error(f"❌ [Deploy-Morph] Error in generate_docker: {e}")
            return False
