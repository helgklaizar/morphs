...
EnterWorktree/ExitWorktree tools for compilation isolation:
- EnterWorktree: creates a git worktree in /tmp, switches to an isolated branch
- ExitWorktree: applies the diff back or discards it, cleans up the worktree
"""
import os
import subprocess
import tempfile
...
class WorktreeRegistry:
    """Global registry of active worktree sessions."""
    _sessions: dict[str, dict] = {}  # session_id -> {branch, wt_path, original_cwd, base_path}


class WorktreeTools:
    """
    Task 33 — Git Worktree Management for isolated compilation/builds.
    Does not break the project's working directory during code experiments.
    """

    @staticmethod
    def _run_git(cmd: list[str], cwd: str) -> tuple[bool, str]:
        """Runs a git command, returns (success, output)."""
        try:
    ...
    def enter_worktree(
        ...
        """
        [EnterWorktree] Creates an isolated git worktree in a temporary folder.
        
        Args:
            project_path: path to the project (must be a git repository)
            branch_name: name of the new branch (auto-generated if None)
            base_branch: base branch/commit for the worktree (defaults to HEAD)
        
        Returns:
            session_id to be used in ExitWorktree, or an error string.
        """
        ...
        # Create a temporary folder for the worktree
    ...
    def get_worktree_path(session_id: str) -> str:
        """
        Returns the path to the worktree directory for the specified session.
        Use this path when calling build tools instead of the project root.
        """
    ...
    def commit_worktree_changes(session_id: str, commit_message: str) -> str:
        """
        [WorktreeCommit] Commits all changes to the worktree branch.
        """
        if session_id not in WorktreeRegistry._sessions:
            return f"Error: Session '{session_id}' not found."
    ...
    def merge_worktree_to_main(session_id: str, target_branch: str = "main") -> str:
        """
        [WorktreeMerge] Merges changes from the worktree branch into target_branch.
        The worktree must be committed before merging.
        """
        if session_id not in WorktreeRegistry._sessions:
            return f"Error: Session '{session_id}' not found."
        ...
        # Create a patch from the changes in the worktree branch
    ...
    def exit_worktree(session_id: str, apply_changes: bool = False, target_branch: str = "main") -> str:
        """
        [ExitWorktree] Finishes a worktree session: applies or discards changes.
        
        Args:
            session_id: Session ID from enter_worktree
            apply_changes: if True — merges changes into target_branch before deletion
            target_branch: branch to merge into (when apply_changes=True)
    ...
    def list_worktrees(project_path: str = ".") -> str:
        """Shows all active git worktrees for the project."""
        project_path = str(Path(project_path).resolve())
        git_root = WorktreeTools._get_git_root(project_path)
        if not git_root:
