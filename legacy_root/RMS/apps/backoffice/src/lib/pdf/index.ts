...
import { SupplierOrder } from "@/store/useSupplierProductsStore";
...
export class PDFGenerator {
  ...
  static generateSupplierOrder(order: SupplierOrder) {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    const supplier = order.expand?.supplier;
    ...
    doc.text(`Date: ${format(new Date(order.created || new Date()), "dd.MM.yyyy HH:mm")}`, 20, 46);
    doc.text(`Via: ${order.sent_via}`, 20, 52);
    ...
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.text(`TOTAL AMOUNT: ${(order.total_amount || 0).toFixed(2)} NIS`, pageWidth - 20, finalY, { align: "right" });
