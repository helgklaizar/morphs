...
import { useSupplierProductsStore } from '../useSupplierProductsStore';
import { SupplierProductsRepository } from '@/lib/repositories/supplierProducts';

vi.mock('@/lib/repositories/supplierProducts', () => ({
...
describe('useSupplierProductsStore', () => {
  ...
  it('addProduct triggers refetch if supplier_id present', async () => {
    const rfMock = vi.fn();
    useSupplierProductsStore.setState({ fetchProducts: rfMock });

    await useSupplierProductsStore.getState().addProduct({ name: 'Milk', supplier_id: 'sup1' });
