...
const columns: OrderStatus[] = ['new', 'preparing', 'ready', 'delivering', 'completed'];
...
export default function OrdersPage() {
  ...
  const [viewMode, setViewMode] = useState<"regular" | "mega" | "kitchen">("regular");
  const [statusFilter, setStatusFilter] = useState<OrderStatus | 'all' | 'weekly_today'>('all');
  ...
  const handleDeleteSubscription = async () => {
    ...
    try {
      for (const order of confirmDeleteSubscription.orders) {
        await useOrdersStore.getState().deleteOrder(order.id);
      }
    } catch (err: any) {
      useToastStore.getState().error("Ошибка при удалении: " + err.message);
    } finally {
    ...
    try {
      for (const order of profile.orders) {
        if (order.status === 'pending') {
          await useOrdersStore.getState().updateOrderStatus(order.id, 'new');
        }
      }
    } catch (err: any) {
      useToastStore.getState().error("Ошибка при принятии в работу: " + err.message);
    }
  };
  ...
  // Group subscriptions into Mega Profiles by phone and name
...
function OrderCard({ order, onEdit, onArchive, onDelete }: { order: Order; onEdit: () => void; onArchive: () => void; onDelete: () => void }) {
  ...
  const isDelivery = order.customerName.toLowerCase().includes("доставка") || !isSubscription; // fallback generic logic
  ...
  return (
    <div className="flex flex-col bg-[#141414] rounded-[18px] border h-full shadow-lg group hover:border-white/20 transition-all" style={{ borderColor: `${c.hex}33` }}>
      {/* Header Stripe */}
      <div className="flex items-center px-4 py-3 rounded-t-[17px] gap-2 overflow-hidden border-b border-white/5" style={{ backgroundColor: `${c.hex}08` }}>
        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: c.hex, boxShadow: `0 0 6px ${c.hex}80` }}></div>
        <div className="flex flex-col">
          <span className="text-[10px] font-black uppercase tracking-widest opacity-40 leading-none mb-0.5" style={{ color: c.hex }}>Заказ</span>
          <span className="text-sm font-black tracking-tighter" style={{ color: c.hex }}>#{order.id.slice(-5).toUpperCase()}</span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
            <button 
              onClick={() => PDFGenerator.generateOrderInvoice(order)}
              className="p-1.5 rounded-lg bg-white/5 text-white/40 hover:text-orange-500 hover:bg-orange-500/10 transition-all"
              title="Cкачать Накладную (PDF)"
            >
              <FileText className="w-4 h-4" />
            </button>
            <button 
              onClick={onArchive}
              className="p-1.5 rounded-lg bg-white/5 text-amber-500 hover:bg-amber-500/10 transition-all"
              title="В архив"
            >
              <Archive className="w-4 h-4" />
            </button>
            <button onClick={onEdit} className="p-1.5 rounded-lg bg-white/5 text-white/40 hover:text-white hover:bg-white/10 transition-all">
              <Edit2 className="w-4 h-4" />
            </button>
            <button 
              onClick={onDelete}
              className="p-1.5 rounded-lg bg-white/5 text-red-500/40 hover:text-red-500 hover:bg-red-500/10 transition-all"
            >
      ...
      <div className="flex flex-col flex-1 p-4 pt-4">
        {/* Customer Info */}
        <div className="mb-4 border-b border-white/5 pb-4 flex justify-between items-start gap-4">
          <div className="min-w-0 flex-1">
            <h3 className="font-black text-xl text-white truncate leading-none mb-1.5 uppercase tracking-tight">
              {cleanName}
            </h3>
            <p className="text-sm font-bold text-white/40 tracking-wider font-mono">{order.customerPhone || 'НЕТ НОМЕРА'}</p>
            {isSubscription && (
              <span className="mt-2 inline-flex border border-green-500/30 bg-green-500/10 text-green-500 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{subDayInfo}</span>
            )}
          </div>
          <div className={`px-2.5 py-1 rounded-xl font-bold text-[10px] uppercase tracking-widest border transition-colors shrink-0 ${
        ...
        <div className="flex-1 space-y-2 mb-6">
          <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mb-2">Состав заказа</p>
          {order.items.map(it => (
            <div key={it.id} className="flex items-center justify-between text-sm bg-white/2 p-2 rounded-xl border border-white/0 hover:border-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-black flex items-center justify-center font-black text-orange-500 text-xs border border-white/5">
                  {it.quantity}
                </div>
                <span className="font-bold text-white/80 line-clamp-1">{it.menuItemName}</span>
              </div>
              <span className="text-white/30 font-mono text-[11px]">{(it.priceAtTime || 0) * it.quantity} ₪</span>
            </div>
          ))}
        </div>

        {/* Footer info with Price Highlight */}
        <div className="flex items-end justify-between mt-auto pt-4 border-t border-white/5">
          <div className="flex flex-col gap-1 min-w-0 pr-2">
            <div className="flex items-center gap-1.5 text-white/30">
              <Clock className="w-3 h-3" />
              <span className="text-[10px] font-black uppercase tracking-widest leading-none">{deliveryTypeLabel}</span>
            </div>
            <span className="text-sm font-black text-orange-500/60 leading-none">{pickupStr}</span>
            {isDelivery && addressStr && (
               <span className="text-xs font-bold text-white/50 mt-1 truncate max-w-[200px]" title={addressStr}>
                 📍 {addressStr}
               </span>
            )}
          </div>
          ...
          <div className="flex flex-col items-end shrink-0">
             {isSubscription ? (
               <>
                 <p className="text-[10px] font-black text-green-500/70 uppercase tracking-widest mb-0.5">Мега-заказ</p>
                 <span className="text-xl font-black text-green-500 leading-none tracking-tighter uppercase relative top-0.5">
                   Оплачено
                 </span>
               </>
             ) : (
               <>
                 <p className="text-[10px] font-black text-white/30 uppercase tracking-widest opacity-50 mb-0.5">К оплате</p>
                 <span className="text-3xl font-black text-white leading-none tracking-tighter">
                    {order.totalAmount} <span className="text-orange-500 text-xl font-bold ml-0.5">₪</span>
                 </span>
               </>
             )}
          </div>
