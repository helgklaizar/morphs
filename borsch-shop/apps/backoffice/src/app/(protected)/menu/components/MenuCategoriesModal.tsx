...
export function MenuCategoriesModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from('menu_categories').insert({ name: newTitle.trim(), order_index: maxOrder + 1 });
    if (!error) {
      setNewTitle("");
      fetchCategories();
    } else {
  ...
  const executeDelete = async () => {
    ...
    const { error } = await supabase.from('menu_categories').delete().eq('id', confirmDelete.id);
    if (!error) {
      fetchCategories();
