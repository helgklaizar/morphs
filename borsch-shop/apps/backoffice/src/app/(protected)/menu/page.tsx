...
export default function MenuPage() {
  ...
  }, [fetchMenuItems, fetchCategories, fetchRecipes, fetchAssemblies, fetchSettings, subscribeToMenu, unsubscribeFromMenu]);
      ...
      <div className="flex-1 overflow-auto pb-10">
        {isLoading && items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
            {items.filter(i => (filterPolls ? i.isPoll : !i.isPoll)).map(item => (
    ...
    <div className="flex flex-col bg-[#141414] rounded-2xl border border-white/10 overflow-hidden group hover:border-white/20 transition-colors">
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs font-semibold text-green-400 bg-green-400/10 px-2 py-0.5 rounded">Себест: {item.cost.toFixed(2)} ₪</span>
        </div>
