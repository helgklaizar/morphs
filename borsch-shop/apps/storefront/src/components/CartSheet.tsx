...
export default function CartSheet({ landingSettings }: { landingSettings?: Record<string, unknown> }) {
  ...
  const [lastOrderItems, setLastOrderItems] = useState<Record<string, unknown>[]>([]);
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (order?.id) {
        ...
        // Telegram notification is now handled automatically by the PocketBase backend hook (backend/pb_hooks/main.pb.js)
        // whenever a new order is inserted into the 'orders' collection.
      }
        ...
        {success ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-6 overflow-y-auto">
            ...
            {type === "delivery" && orderId && (
              <a href={`https://t.me/borschordersbot?start=${orderId}`} target="_blank" rel="noopener noreferrer" className="mt-2 w-full flex flex-row items-center justify-center gap-3 bg-[#0088CC] hover:bg-[#0077b5] text-white font-bold py-4 rounded-2xl transition shadow-lg">
            ...
            {lastOrderItems.length > 0 && (
              <div className="w-full bg-white/5 rounded-3xl border border-white/5 p-5 text-left space-y-3 mt-4 animate-in fade-in duration-300">
                ...
                <div className="space-y-2.5 max-h-[140px] overflow-y-auto pr-2 custom-scrollbar">
                  {lastOrderItems.map((item: Record<string, any>) => (
                    ...
                    {items.map((item) => (
                      <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} key={item.id} className="flex gap-4 bg-zinc-900/60 p-3 rounded-[1.5rem] border border-white/5 items-center relative overflow-hidden group">
                        ...
                        <div className="w-20 h-20 relative rounded-2xl bg-zinc-800 overflow-hidden shrink-0 border border-white/5">
                            {item.image_url ? <Image src={item.image_url} alt={item.name} fill className="object-cover" /> : <div className="w-full h-full flex items-center justify-center text-brand font-black text-2xl">{item.name.charAt(0)}</div>}
                        </div>
            ...
            {items.length > 0 && (
              <div className="p-6 bg-zinc-950/90 border-t border-white/5 space-y-5 rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.8)] relative z-20">
                ...
                <div className="flex gap-2">
                  {landingSettings?.is_delivery_enabled && (
                  ...
                  {landingSettings?.is_pickup_enabled && (
