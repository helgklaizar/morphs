...
export async function getDb(): Promise<Database> {
  ...
  if (typeof window !== 'undefined' && !(window as any).__TAURI_INTERNALS__) {
    throw new Error("⚠️ Ошибка: Ты открыл админку в Google Chrome/Safari по ссылке localhost:3000. Локальная SQLite база данных работает ТОЛЬКО внутри десктопного окна Borsch Shop (Tauri). Закрой вкладку браузера и открой отдельное окно приложения (оно в доке/таскбаре)!");
  }
  
  if (!dbInstance) {
    dbInstance = await Database.load("sqlite:borsch_shop.db");
  }
