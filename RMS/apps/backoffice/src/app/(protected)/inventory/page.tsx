...
export default function InventoryPage() {
  ...
  const ai = useAiAdvisor('inventory');
  ...
  }, [fetchInventory, subscribeToInventory, unsubscribeFromInventory, ai.fetchInsights]);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex bg-[#141414] p-1.5 rounded-xl border border-white/5 shadow-inner mb-6 shrink-0 w-fit">
        <Link href="/inventory" className="px-5 py-2 font-bold text-sm transition-all rounded-lg flex items-center gap-2 bg-orange-500 text-white shadow-md shadow-orange-500/20">
          Категории
        </Link>
                            ...
                            {col.items.map(item => (
                              <div key={item.id} className="bg-[#242424] rounded-lg p-3">
                                <div className="flex items-start justify-between mb-2">
                                  ...
                                  <span className="text-xs text-orange-400 font-medium whitespace-nowrap ml-2">{item.price} ₪/{item.unit}</span>
                                </div>
