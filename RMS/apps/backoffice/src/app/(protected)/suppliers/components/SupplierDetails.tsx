              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {products.map(p => (
                  <div key={p.id} className="bg-[#1C1C1C] rounded-2xl p-4 border border-white/5 hover:border-white/20 transition flex flex-col h-full">
                     <h3 className="font-bold text-sm mb-2 leading-tight flex-1">{p.name}</h3>
                     ...
                     <div className="flex gap-2 shrink-0">
                        ...
                        <button onClick={() => setConfirmDelete({ id: p.id, name: p.name })} className="w-8 h-8 flex justify-center items-center rounded-lg text-white/20 hover:text-red-400 hover:bg-red-500/10 shrink-0"><Trash2 className="w-4 h-4" /></button>
                     </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2 max-w-2xl">
                {orders.map(o => (
                  <div key={o.id} className="bg-[#1C1C1C] rounded-xl p-4 border border-white/5 flex gap-4 pr-6 shrink-0 text-left">
                     ...
                     <div className="flex-1">
                        <h4 className="font-bold mb-1">Заказ от {new Date(o.created).toLocaleString()}</h4>
                        <div className="text-xs text-white/60 mb-2">Отправлено: {o.sent_via} • Статус: {o.status}</div>
                     </div>
                  </div>
                ))}
              </div>
              ...
              <button 
                onClick={() => {
                  if (newProductName.trim()) {
                    addProduct({ supplier_id: id, name: newProductName.trim(), prices: {"шт": 0} });
                    setNewProductName("");
