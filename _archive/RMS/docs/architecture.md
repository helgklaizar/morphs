# RMS AI OS — Архитектурный Граф

В данном документе представлена наглядная архитектурная схема ядра `rms-ai-os` в парадигме Offline-First.
...
flowchart TD
    ...
    subgraph Cloud["☁️ Облако (rms.shop)"]
    ...
    %% Web Landing
    subgraph Landing["📱 Landing (Next.js)"]
...
## Conflict Resolution (Архитектурное решение)

Так как терминалы могут работать офлайн, возникает проблема синхронизации состояний (`sync_status != 'synced'`). 
Базовый паттерн разрешения конфликтов (Conflict Resolution) в RMS AI OS:
- **Last-Write-Wins (LWW) на основе `updated_at`**: Запись с более свежим timestamp всегда перетирает более старую на уровне PocketBase (сервер сверяет приходящий timestamp с текущим).
- **Идемпотентность статусов**: Заказы могут двигаться только вперед по пайплайну (`new -> preparing -> ready -> completed`). Нельзя перевести `completed` обратно в `new`. Если локальная касса офлайн-закрыла заказ, а облако еще думает, что он `new` — при пуше от кассы статус жестко обновится до `completed`.
- **Soft Deletes**: Физическое удаление запрещено (кроме черновиков). Вместо удаления используется смена флага `is_archived = 1`. Это гарантирует, что удаление из офлайна не породит "мертвых", неотслеженных записей на сервере.
