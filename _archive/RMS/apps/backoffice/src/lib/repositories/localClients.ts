...
export class LocalClientsRepository {
  ...
  static async add(payload: Partial<ClientRecord>): Promise<void> {
    ...
    await recordOutboxEvent(db, 'clients', 'client_add', { id, ...payload });
  }
  ...
  static async update(id: string, payload: Partial<ClientRecord>): Promise<void> {
    ...
    if (fields.length > 0) {
      ...
      await recordOutboxEvent(db, 'clients', 'client_update', { id, ...payload });
    }
  }
  ...
  static async delete(id: string): Promise<void> {
    ...
    await recordOutboxEvent(db, 'clients', 'client_delete', { id });
  }
}
